...
const useWalletLizards = (refresh) => {
  console.log("useWalletLizards ran");
  ...
  useEffect(() => {
    (async () => {
      ...
      const nftsForOwner = await getLizardsForOwner(connection, wallet.publicKey);
...
export default useWalletLizards;
